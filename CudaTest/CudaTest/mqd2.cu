...
__global__ void
mqd2_CUDA(double** result, double *TP, int TPx, int TPy, double *CN, int CNx, int CNy, double *A, int Ax, int Ay, double *C, int Cx, int Cy)
{
	int i = blockDim.y * blockIdx.y + threadIdx.y;
	int j = blockDim.x * blockIdx.x + threadIdx.x;
	__syncthreads();
	if (i == 0 & j == 0)
	{
		...
		mqd22_CUDA << <1, 1 >> > (D, Dt, Dx, Dxx, TP, dimTP, CN, dimCN, A, dimA, C, dimC);
		gpuErrchk << <1, 1 >> >(cudaPeekAtLastError());
		gpuErrchk << <1, 1 >> >(cudaDeviceSynchronize());
